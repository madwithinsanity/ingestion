...
import com.madwithinsanity.utilities.ResourceUtils;
...
public class FileRepositoryImpl implements FileRepository {
	...
	public String save(File file) throws IOException {
		...
		GridFSFile gridFile = gridTemplate.store(fileStream, file.getName());
		logger.info(gridFile.toString());
		if (fileStream != null) {
			fileStream.close();
		}
		return file.getName();
	...
	public File findOne(String filename) throws IOException {
		if (filename != null) {
			...
			if (gridFSDBFile != null) {
				File outputFile = new File(utils.getDirectory(), filename);
